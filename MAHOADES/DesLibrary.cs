...
namespace MAHOADES
{
    public static class DesLibrary
    {
        ...
        public static string EncrpytionDES(string K, string _M)
        {
            ...
            ListKL[0] = cvtBinToHex(tmtL);
            ListKR[0] = cvtBinToHex(tmtR);
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                string E_R0 = transposition(tmtR, E);//Mở rộng nữa phải
                                                     ...
                                                     //ListKey(K);
                string K1 = ListKey[i];
                string A = XOR_BIT(E_R0, K1);
                //Thế S-BOX
                string SB = SBOX(A);
                //Hoán vị P(B)

                string PB = transposition(cvtHextToBin(SB), P);
                //R1=P(B) xor L0
                R = XOR_BIT(PB, tmtL);

                tmtL = L;
                tmtR = R;
                ListKL[i + 1] = cvtBinToHex(L);
                ListKR[i + 1] = cvtBinToHex(R);

            }
            C = cvtBinToHex(transposition(R + L, IP1));
            return C;
        }
        ...
        static string cvtHextToBin(string hexstring)
        {
        ...
        public static string cvtBinToHex(string strBinary)
        {
        ...
        public static string[] ListKeys(string K)
        {
            ...
            string PC1K = transposition(cvtHextToBin(K), pc1);
        ...
        static string XOR_BIT(string _A, string _B)
        {
